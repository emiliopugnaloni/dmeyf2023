...
PARAM$experimento <- "C3_OptBayesiana"
...
PARAM$input$testing2 <- c(202107)
PARAM$input$validation <- c(202106)
PARAM$input$training <- c(201906, 201907, 201908, 201909, 201910, 201911, 201912, 202008, 202009, 202010, 202011, 202012, 202101, 202102, 202103,202104, 202105)

# un undersampling de 0.1  toma solo el 10% de los CONTINUA
PARAM$trainingstrategy$undersampling <- 1.0
PARAM$trainingstrategy$semilla_azar <- 673787 # Aqui poner su  primer  semilla

PARAM$hyperparametertuning$POS_ganancia <- 273000
PARAM$hyperparametertuning$NEG_ganancia <- -7000

# Aqui poner su segunda semilla
PARAM$lgb_semilla <- 673789
#------------------------------------------------------------------------------
...
EstimarGanancia_lightgbm <- function(x) {
  ...
  # aplico el modelo a testing_2 y calculo la ganancia
  prediccion <- predict(
    modelo_train,
    data.matrix(dataset_test2[, campos_buenos, with = FALSE])
  )

  tbl <- copy(dataset_test2[, list("gan" = ifelse(clase_ternaria == "BAJA+2",
    PARAM$hyperparametertuning$POS_ganancia, 
    PARAM$hyperparametertuning$NEG_ganancia))])
  ...
  ganancia_test2 <- tbl[, max(gan_suavizada, na.rm = TRUE)]

  cantidad_test_normalizada2 <- which.max(tbl[, gan_suavizada])

  rm(tbl)
  gc()

  ganancia_test_normalizada2 <- ganancia_test2
  ...
  xx$estimulos2 <- cantidad_test_normalizada2
  xx$ganancia_2 <- ganancia_test_normalizada2
